...
void mat4_load_rotate_z(struct mat4* out, float degree)
{
    float c = cos(degree);
    float s = sin(degree);
...
{
    struct mat4 tmp;
    mat4_load_translate(&tmp, x, y, z);
    mat4_multiply(out, &tmp, out);
}

void mat4_scale(struct mat4* out, float x, float y, float z)
{
    struct mat4 tmp;
    mat4_load_scale(out, x, y, z);
    mat4_multiply(out, &tmp, out);
}
